...
{ public:
  ...
  public:
   ...
   int Process(SampleBuffer<int16_t> &Buffer, int16_t Bias=0)
   { PulseBox.Clear(); Pulses=0;
     if(Threshold<=0) return 0;
     int Samples = Buffer.Samples();
     if(Samples<PulseBoxSize) return 0;
   ...
   static void SetZero(int16_t *Data, int16_t Bias=0)
   { Data[0]=Bias; Data[1]=Bias; }

   static void SetHalf(int16_t *Data, int16_t Bias=0)
   { int16_t I = Data[0]-Bias; Data[0] = Bias + (I>>1);
     int16_t Q = Data[1]-Bias; Data[1] = Bias + (Q>>1); }
